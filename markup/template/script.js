...
function scrollWindow(e) {
  $('form.i-invisible')
    ...
    .each(function () {
      ...
      var extra = 250;
      var scrollToScreen =
        bottom - document.documentElement.clientHeight + extra;
...
$('.bj-hidden-link').click(function (e) {
  e.preventDefault();
  var c = $(this);
  return c.hasClass('i-up')
    ? void c.removeClass('i-up').parent().find('.bj-hidden__hidden').slideUp()
    : void c.addClass('i-up').parent().find('.bj-hidden__hidden').slideDown();
});
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.success === 'success') {
          $('.bj-news-subscribe__s .alert-success').addClass('hide');
          $('.bj-news-subscribe__s .alert-warning').addClass('hide');
          $('.bj-news-subscribe__s .bj-news-subscribe-form').removeClass(
            'hide'
          );
        }
      },
...
$('.b-catalog-element__button .btn').bind('add.ecommerce', function () {
  ...
  var productObj = {
    ...
    price: parseInt(
      $element.find('.b-catalog-element__price').text().split(/\s/).join(''),
      10
    ),
...
//create order
const makeOrderButton = document.querySelector('.bx-soa-button-block a');
